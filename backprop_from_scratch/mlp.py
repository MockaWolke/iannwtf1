...
class MLP(Perceptron):
    ...
    def __init__(self, input_units = 2, output_width = 1, hidden_width = 4, depth = 2, global_activ_func = sigmoid, global_activ_func_derivate = sigmoidprime, global_learning_rate = 1):
        ...
        # let's create an array and fill it up with on single Perceptron so that we have the correct datatype for our array
        self.hidden_layers = np.full((self.hidden_width,depth),Perceptron(1))
        
        # The broblem now is that all the entries of our hidden_layers array corespond to one single Perceptron, with excatcly the same random values
        # We dont want that so let's change this:
        
        for y in range(self.hidden_width): # here we iterate systematically through our hidden layers, we will do this more often later
    ...
    def forward_step(self, inputs):
        ...
        self.prediction = np.nan # storage of prediction(s)

        # let's compute the forward step for every perceptron in our hidden_layer, so again we iterate through our whole hidden layer
        for x in range(self.depth):
